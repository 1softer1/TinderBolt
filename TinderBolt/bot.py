...
from gpt import *
from util import *
from TOKEN import TOKENAI
from TOKEN import TOKENBOT
...
async def start(update, context,):
    ...
    await show_main_menu(update, context,{
        ...
        "opener": "Зсообщение для знакомства 🥰",
...
async def profile_dialog(update, context):
    ...
    if dialog.count == 1:
        ...
        my_mesage = await send_text(update, context, "Chatgpt думает")
        answer = await chatgpt.send_question(prompt, user_info)
        await my_mesage.edit_text(answer)
...
async def opener_dialog(update, context):
    ...
    if dialog.count == 1:
        ...
        dialog.user["age"] = text
        await send_text(update, context, "ее внешность 1-10")
    elif dialog.count == 3:
...
chatgpt = ChatGptService(token = TOKENAI)
